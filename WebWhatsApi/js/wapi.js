...
window.WAPI.leaveGroup = function (groupId) {
    ...
    let group = window.Store.Chat.get(groupId);
...
window.WAPI.sendImageFromDatabasePicBot = function (picId, chatId, caption) {
    let chatDatabase = window.WAPI.getChatByName('DATABASEPICBOT');
    let msgWithImg = chatDatabase.msgs.find((msg) => msg.caption === picId);
    if (msgWithImg === undefined) {
        return false;
    }
    let chatSend = WAPI.getChat(chatId);
...
window.WAPI.ReplyMessage = function (idMessage, message, done) {
    let messageObject = window.Store.Msg.get(idMessage);
...
window.WAPI.sendMessageToID = function (id, message, done) {
    try {
        let idUser = new window.Store.UserConstructor(id);
        // create new chat
...
window.WAPI.getNewMessageId = function (chatId) {
    let newMsgId = Store.Msg.models[0].id.clone();
...
window.WAPI.sendVCard = function (chatId, vcard) {
    ...
    let tempMsg = Object.create(Store.Msg.models.filter(msg => msg.__x_isSentByMe)[0]);
    let newId = window.WAPI.getNewMessageId(chatId);
